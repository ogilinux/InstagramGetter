...
def Print_Logo():
	...
	{Y} ▪   ▐ ▄ .▄▄ · ▄▄▄▄▄ ▄▄▄·   ·▄▄▄      ▄▄▄   ▄▄· ▄▄▄ .
        ...
        {Y}▀▀▀▀▀ █▪ ▀▀▀▀  ▀▀▀  ▀  ▀   ▀▀▀  ▀█▄▀▪.▀  ▀·▀▀▀  ▀▀▀ 
		     ...
		     {U}{Y}~ {G}Information Getter {M}- {G}By .::ogilinux::. {Y}~{UR}'''.format(Y=YELLOW,M=MAGENTA,G=GREEN,GR=GRAY,C=CYAN,U=UNDERLINE,UR=RUNDERLINE)
...
def Clear():
	if os.name == 'nt':
		os.system('cls')
		os.system('title Information Getter - By .::ogilinux::.')
	else:
		os.system('clear')
...
def Get(user):
    ...
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Korisnicko ime'+BLUE+': '] = Username
    ...
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Followera'+BLUE+': '] = Followers
    ...
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Followuje'+BLUE+': '] = Following
    Posts = ''
    ...
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Postovi'+BLUE+': '] = Posts
    shit = {}
    ...
    Type = RED+str(al['t'])
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Tip'+BLUE+': '] = Type
    Name = RED+Decoder(str(al['n']))
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Ime'+BLUE+': '] = Name
    Des = RED+Decoder(str(al['d']))
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Deskripcija'+BLUE+': '] = Des
    ...
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Business Nalog'+BLUE+': '] = Business_Account
    Joined_Recently = RED+str(All3[0][1]).replace('false','False').replace('true','True')
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Datum ulaska'+BLUE+': '] = Joined_Recently
    Private = RED+str(All3[0][3]).replace('false','False').replace('true','True')
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Privatni nalog'+BLUE+': '] = Private
    Verified = RED+str(All3[0][4]).replace('false','False').replace('true','True').split(',')[0]
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'Verifikacija'+BLUE+': '] = Verified
    Pic = RED+short(str(re.findall('<meta property="og:image" content="(.*)" />',r)[0]))
    ret[YELLOW+'['+MAGENTA+'#'+YELLOW+']'+GREEN+'URL Slike'+BLUE+': '] = Pic
    return ret

def Main():
	Clear()
	Print_Logo()
	print(YELLOW+' ['+MAGENTA+'*'+YELLOW+']'+GREEN+'Upisi ime naloga')
	...
	for l in info:
		print('    '+l+info[l])
	print(YELLOW+'\n\n ['+MAGENTA+'!'+YELLOW+']'+GREEN+'Zavrseno.')					
	input('{Y}  [{M}!{Y}]{G}Pritisni `{B}Enter{G}` Da zavrsis...'.format(Y=YELLOW,M=MAGENTA,G=GREEN,B=BLUE))
